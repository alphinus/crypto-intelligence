            ...
            <div className="flex items-start gap-2">
              ...
              {sentimentSignal && (
                <div className="flex flex-col items-end text-[10px]">
                  <span className={`px-2 py-0.5 rounded ${sentimentSignal.direction === 'bullish' ? 'bg-green-500/20 text-green-400' :
      ...
      <AnimatePresence>
        {sentimentSignal && !sentimentConflict?.hasConflict && sentimentMode === 'info' && (
          <motion.div
            ...
            animate={{ opacity: 1, height: 'auto' }}
            exit={{ opacity: 0, height: 0 }}
            className="mb-3 p-2 bg-gray-100 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-lg"
          >
            <div className="flex items-center justify-between text-xs">
              <span className="text-gray-400">Sentiment Signal:</span>
              <div className="flex items-center gap-2">
                <span className={`px-2 py-0.5 rounded font-medium ${sentimentSignal.direction === 'bullish' ? 'bg-green-500/20 text-green-400' :
                    sentimentSignal.direction === 'bearish' ? 'bg-red-500/20 text-red-400' :
                      'bg-gray-500/20 text-gray-400'
                  }`}>
                ...
                <span className={`text-[10px] ${sentimentSignal.confidence === 'high' ? 'text-green-400' :
        ...
        {TIMEFRAME_CONFIG.filter(c => !c.hidden).map((config, index) => {
          ...
          return (
            <motion.div
              ...
              tabIndex={0}
              className={`relative bg-white dark:bg-gray-900/50 border rounded-lg overflow-hidden cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-400 ${isBestTrade
            ...
            >
              {/* Header with Rank */}
              <div className="bg-gray-100 dark:bg-gray-800/50 px-3 py-2 border-b border-gray-200 dark:border-gray-700/50">
                    ...
                    {score && setup.type !== 'wait' && (
                      <div className="mb-2">
                        <div className="flex items-center justify-between mb-1">
                          ...
                          <span className="text-xs font-bold text-gray-900 dark:text-white">{score.total}/100</span>
                        </div>
                    ...
                    {setup.type !== 'wait' ? (
                      <>
                        ...
                        <div className="space-y-1 text-xs">
                          ...
                          <div className="flex justify-between border-t border-gray-200 dark:border-gray-700/50 pt-1 mt-1">
              ...
              <AnimatePresence>
                {hoveredCard === config.timeframe && score && setup?.type !== 'wait' && (
                  <motion.div
                    ...
                    className="absolute inset-0 bg-white/95 dark:bg-gray-900/95 p-2 flex flex-col justify-center z-10 overflow-y-auto">
                    {/* Score Summary Bar */}
                      ...
                      <div className="bg-blue-900/20 rounded p-1.5">
                        ...
                        <div className="space-y-0.5">
                          <div className="flex justify-between">
                            ...
                            <span className="text-gray-900 dark:text-white">{score.components.trendAlignment}/25</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-500">Momentum:</span>
                            <span className="text-gray-900 dark:text-white">{Math.round(score.components.momentumStrength)}/20</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-500">RR:</span>
                            <span className="text-gray-900 dark:text-white">{score.components.riskRewardQuality}/20</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-500">Konfl:</span>
                            <span className="text-gray-900 dark:text-white">{score.components.confluenceBonus}/15</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-500">Vol:</span>
                            <span className="text-gray-900 dark:text-white">{score.components.volumeConfirmation}/10</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-500">Level:</span>
                            <span className="text-gray-900 dark:text-white">{score.components.levelProximity}/10</span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="bg-purple-900/20 rounded p-1.5">
                        ...
                        <div className="space-y-0.5">
                          <div className="flex justify-between">
                            ...
                            <span className="text-gray-900 dark:text-white">{score.components.fearGreedAlignment || 0}/25</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-500">Social:</span>
                            <span className="text-gray-900 dark:text-white">{Math.round(score.components.socialSentiment || 0)}/25</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-500">Funding:</span>
                            <span className="text-gray-900 dark:text-white">{score.components.fundingRateBias || 0}/25</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-500">Markt:</span>
                            <span className="text-gray-900 dark:text-white">{score.components.marketMomentum || 0}/25</span>
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    {/* Reasoning */}
                    {setup?.reasoning && (
                      <p className="text-[8px] text-gray-500 dark:text-gray-400 mt-1.5 text-center italic border-t border-gray-200 dark:border-gray-700 pt-1">
                        {setup.reasoning}
                      </p>
                    )}
                  </motion.div>
